        ...
        try {
            ...
            if (autoCaption) {
                ...
                if (!translate || targetLanguages.length === 0) {
                    ...
                    let translationsByFile;
                    try {
                        translationsByFile = await translationService.translateCaptionsBatch(baseCaptions, targetLanguages, selectedModel);
                    ...
                    for (const [file, baseCaption] of Object.entries(baseCaptions)) {
                        captions[file] = {};
                        captions[file][lang] = baseCaption;
...
`)
        ...
        try {
            const { device, lang, langs, model, dryRun } = options;
            const parent = command?.parent;
            const parentOpts = parent?.opts?.() ?? {};
            const parentDevice = parentOpts.device;
            const resolvedDevice = device ?? parentDevice;
            const resolvedLang = command?.getOptionValueSource?.('lang') === 'default' && parentOpts.lang
                ? parentOpts.lang
                : (lang ?? 'en');
            const resolvedModel = command?.getOptionValueSource?.('model') === 'default' && parentOpts.model
                ? parentOpts.model
                : (model ?? 'gpt-5-mini');
            const resolvedLangs = (langs ?? parentOpts.langs);
            const resolvedDryRun = (dryRun ?? parentOpts.dryRun ?? false);
            if (!resolvedDevice) {
                console.error(pc.red('Error:'), 'Missing required option: --device <name>');
                process.exit(1);
            }
            ...
            const targetLanguages = resolvedLangs
                ? resolvedLangs.split(',').map((l) => l.trim()).filter(Boolean)
                : [resolvedLang];
            ...
            const deviceEntry = config.devices?.[deviceKey];
            if (!deviceEntry) {
            ...
            const { width: outputWidth, height: outputHeight } = await resolveOutputSize(inputDir, resolution, deviceKey);
            const layout = config.layout || 'header';
            const constraintLayout = layout === 'screenshot-only' ? 'header' : layout;
            const strategy = getDeviceStrategyV2(deviceKey);
...
async function resolveOutputSize(inputDir, resolution, deviceType) {
    if (resolution) {
...
function estimateMaxChars(regionWidth, regionHeight, fontSize, maxLines) {
