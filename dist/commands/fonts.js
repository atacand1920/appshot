...
export default function fontsCmd() {
 ...
 */
async function handleSetFont(fontName, deviceName, fontService) {
    try {
 ...
 */
async function handleSelectFont(deviceName, fontService) {
    try {
        ...
        if (selectedFont === '__custom__') {
            const customAnswer = await inquirer.prompt([{
                    ...
                    validate: async (input) => {
                        if (!input.trim())
                            return 'Font name cannot be empty';
