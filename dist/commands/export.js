...
import { mapLanguages } from '../services/fastlane-language-mapper.js';
import { getAllLanguages, organizeScreenshots } from '../services/screenshot-organizer.js';
import { validateExport, validateOutputDirectory, isSafeToClean } from '../services/export-validator.js';
import { generateDeliverfile, generateFastfileLane, generateFastlaneReadme, generateFastlaneGitignore } from '../services/fastlane-config-generator.js';
  ...
  $ cd fastlane && fastlane deliver`)
        ...
        try {
            ...
            if (opts.devices) {
                ...
                const requestedDevices = opts.devices.split(',').map((d) => String(d).trim().toLowerCase());
                deviceFilter = requestedDevices.filter((d) => validDevices.includes(d));
                // Warn about invalid devices
                const invalidDevices = requestedDevices.filter((d) => !validDevices.includes(d));
            ...
            if (opts.langs) {
                detectedLanguages = opts.langs.split(',').map((l) => l.trim());
                if (!opts.json) {
            ...
            if (opts.dryRun) {
                ...
                if (opts.json) {
                    ...
                    if (dryRunResult.actions && dryRunResult.actions.length > 0) {
                        ...
                        dryRunResult.actions.slice(0, sampleSize).forEach((action) => {
                    ...
                    if (dryRunResult.actions && dryRunResult.actions.length > 0) {
                        ...
                        const devicesInActions = new Set(dryRunResult.actions.map((a) => a.device));
                        console.log('\n' + pc.bold('Summary:'));
            ...
            if (opts.clean) {
                ...
                const cleanBrokenSymlinks = async (targetPath) => {
            ...
            if (opts.generateConfig) {
                ...
                const deliverfileContent = generateDeliverfile(Array.from(languageMap.values()), path.relative(fastlaneDir, outputPath));
