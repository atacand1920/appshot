...
async function listImagesRecursively(root) {
...
async function processSingleFile(inputPath, framesDir, options) {
    ...
    let device = options.device || detectDeviceTypeFromDimensions(width, height);
    if (!device) {
        console.error(pc.red('  ✗'), path.basename(inputPath), pc.dim(`Unable to detect device type (${width}x${height}). Use --device to specify.`));
        return { processed: 0, errors: 1 };
    }
    // Choose frame
    const { frame, metadata } = await autoSelectFrame(inputPath, framesDir, device, undefined, Boolean(options.dryRun));
    if (!metadata) {
...
${pc.bold('Examples:')}
  ...
  $ appshot frame ./screenshots --dry-run --verbose`)
        ...
        const validFrameTones = ['original', 'neutral'];
        if (!validFrameTones.includes(frameToneInput)) {
