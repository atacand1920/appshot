            ...
            if (opts.list || (!opts.preview && !opts.sample && !opts.apply)) {
                ...
                for (const category of categories) {
                    ...
                    for (const gradient of categoryGradients) {
                        ...
                        console.log(`  ${pc.cyan(gradient.id.padEnd(12))} ${colorBlocks} ${directionIcon}  ${pc.dim(gradient.description)}`);
                    }
                    console.log();
                }
  ...
  <div class="category">
    ...
    <div class="gradients">`;
        for (const gradient of gradients) {
            const colorBlocks = gradient.colors.map(color => `<div class="color-block" style="background: ${color}"></div>`).join('');
            html += `
      <div class="gradient">
