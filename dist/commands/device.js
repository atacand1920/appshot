    ...
    cmd
        ...
        try {
            ...
            if (options.all) {
                ...
                selectedDevices = devices.filter(d => d.type === 'physical' ? d.state === 'connected' : true);
...
async function findDevice(devices, query) {
    // Try exact match
...
async function captureFromDevice(device, options) {
    ...
    try {
        ...
        const targetPath = await screenshotRouter.routeAndMove(device, tempPath, options.screenName);
