    ...
    const cmd = new Command('validate')
        ...
        try {
            ...
            if (isV2Config(config)) {
                ...
                const configuredResolutions = new Map();
...
function checkRequiredPresetsJson(config) {
    const required = getRequiredPresets();
    const configured = new Set();
    const result = {};
    // Extract device types from config
...
function checkRequiredPresetsFromResolutions(configured) {
    const required = getRequiredPresets();
    for (const [category, presets] of Object.entries(required)) {
        if (presets.length === 0)
            continue;
        console.log(pc.cyan(`${category.toUpperCase()}:`));
        const deviceResolutions = configured.get(category) ?? new Set();
        for (const preset of presets) {
...
function checkRequiredPresetsFromResolutionsJson(configured) {
    const required = getRequiredPresets();
    const result = {};
    for (const [category, presets] of Object.entries(required)) {
        if (presets.length === 0)
