...
import { sanitizeDevices, sanitizeLanguages, sanitizePath, validateTemplateId, sanitizeCaption } from '../utils/validation.js';
...
export const presetCommand = new Command('preset')
    ...
    try {
        ...
        const template = templates.find((t) => t.id === resolved.id);
        // Load current config
        ...
        if (options.dryRun) {
            ...
            if (template.background?.gradient) {
                const colors = template.background.gradient.colors;
