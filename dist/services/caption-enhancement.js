...
export class CaptionEnhancementService {
    ...
    async enhance(options) {
        ...
        const systemPrompt = 'You are a product marketing editor. Rewrite the caption to be concise, clear, and compelling ' +
    ...
    async enhanceBatch(options) {
        ...
        const systemPrompt = 'You are a product marketing editor. Rewrite each caption to be concise, clear, and compelling ' +
            'for App Store screenshots. Preserve proper nouns and key terms. Do NOT translate; keep the same language. ' +
            'Return only JSON.';
        const userPrompt = [
            `Language: ${options.language}`,
        ...
        const maxOutputTokens = Math.min(modelConfig.maxTokens, Math.max(512, 80 * Object.keys(options.captions).length));
        const output = await this.requestCaption({
            model: modelConfig.model,
            instructions: systemPrompt,
            input: [{ role: 'user', content: userPrompt }],
    ...
    async requestCaption(options) {
        ...
        const fallbackText = message?.content?.find((content) => content.type === 'output_text' && content.text)?.text;
...
//# sourceMappingURL=caption-enhancement.js.map