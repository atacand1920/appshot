...
export class FontService {
        ...
        try {
            ...
            for (const fontDir of fontDirs) {
                ...
                if (fontFiles.length > 0) {
                    ...
                    if (fontDir === 'DMSans')
                        displayName = 'DM Sans';
                    else if (fontDir === 'OpenSans')
                        displayName = 'Open Sans';
                    else if (fontDir === 'WorkSans')
                    ...
                    const regularFile = fontFiles.find(f => f.toLowerCase().includes('regular') ||
                        (f.toLowerCase().includes('variable') && !f.toLowerCase().includes('italic'))) || fontFiles[0];
                    if (!fontFamilyMap.has(displayName)) {
                        fontFamilyMap.set(displayName, {
                            name: displayName,
                            family: displayName,
     ...
     */
    async getFontStatusWithEmbedded(fontName) {
        const embeddedFonts = await this.getEmbeddedFonts();
