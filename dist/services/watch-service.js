...
export class WatchService {
    ...
    async start(options) {
        ...
        if (options.devices && options.devices.length > 0) {
            ...
            this.devices = options.devices.map(name => {
                const device = allDevices.find(d => d.name.toLowerCase().includes(name.toLowerCase()));
                if (!device) {
    ...
    async watchDirectory(dir, options) {
        ...
        const watcher = fs.watch(dir, { recursive: true }, (eventType, filename) => {
            ...
            if (filename.includes('final/') || filename.includes('.appshot/'))
                return;
    ...
    debounceFileEvent(filepath, callback) {
    ...
    async processScreenshot(filepath) {
        ...
        try {
            ...
            if (this.devices && this.devices.length > 0) {
                ...
                device = this.devices.find(d => pathLower.includes(d.category) ||
            ...
            if (device && this.config) {
                ...
                if (device) {
                    const targetPath = await screenshotRouter.routeAndMove(device, filepath);
