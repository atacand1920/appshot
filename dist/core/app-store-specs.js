...
export const VISION_PRO_PRESETS = [
    {
        id: 'visionpro',
...
export const APPLE_WATCH_PRESETS = [
    {
        id: 'watch-ultra',
 ...
 */
export function validateResolution(width, height, deviceType) {
    ...
    for (const preset of presets) {
        ...
        if (portrait && portrait[0] === width && portrait[1] === height)
            return true;
        if (landscape && landscape[0] === width && landscape[1] === height)
            return true;
    }
    return false;
}
 ...
 */
export function recommendPreset(width, height, deviceType) {
    const presets = ALL_PRESETS[deviceType];
    if (!presets)
        return undefined;
    for (const preset of presets) {
...
//# sourceMappingURL=app-store-specs.js.map