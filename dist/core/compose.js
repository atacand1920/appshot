...
export async function composeAppStoreScreenshot(options) {
    const { screenshot, frame, frameMetadata, caption, captionConfig, gradientConfig, backgroundConfig, deviceConfig, outputWidth, outputHeight, verbose = false } = options;
    ...
    const { captionTopInsetAbove, deviceTopInsetBelow, bottomInset, gapAbove, gapBelow, overlayBottomSpacing } = resolveLayoutSpacing(captionConfig, deviceConfig);
    ...
    if (backgroundConfig) {
        ...
        backgroundBuffer = await renderBackground(canvasWidth, canvasHeight, backgroundConfig, deviceConfig.input // Pass device path for auto-detection
        );
        // Validate background if it's an image
        if (backgroundConfig.warnOnMismatch && backgroundConfig.image) {
            const validation = await validateBackgroundDimensions(backgroundConfig.image, canvasWidth, canvasHeight);
    ...
    if (frame && frameMetadata) {
        ...
        const isIpad2025Portrait = frameMetadata.deviceType === 'ipad' &&
        ...
        const screenBleedBottom = isIpad2025Portrait ? 8 : 0;
...
export async function composeFrameOnly(options) {
    const { screenshot, frame, frameMetadata, outputFormat = 'png', jpegQuality = 92, verbose = false, frameTone = 'original' } = options;
    if (!frame || !frameMetadata) {
...
export function getFontStack(requestedFont) {
    ...
    const normalizedFont = Object.keys(fontMap).find(key => key.toLowerCase() === requestedFont.toLowerCase());
    if (normalizedFont) {
