 ...
 */
export async function renderBackground(width, height, config, devicePath) {
 ...
 */
async function findBackgroundImage(config, devicePath) {
    const candidates = [];
 ...
 */
async function renderImageBackground(imagePath, options) {
    ...
    try {
        ...
        const fitOptions = calculateFitOptions({ width: metadata.width, height: metadata.height }, { width, height }, fit, position);
        // Apply the fit
        if (fit === 'cover') {
 ...
 */
async function renderSolidBackground(width, height, color) {
 ...
 */
function calculateFitOptions(source, target, fit, position) {
    const positionMap = {
        'center': 'center',
        'top': 'top',
        'bottom': 'bottom',
 ...
 */
export async function validateBackgroundDimensions(imagePath, targetWidth, targetHeight) {
    try {
        ...
        if (metadata.width < targetWidth || metadata.height < targetHeight) {
            warnings.push(`Background smaller than target resolution (${metadata.width}x${metadata.height} < ${targetWidth}x${targetHeight})`);
            warnings.push('Image will be stretched or upscaled, which may reduce quality');
        }
        // Check aspect ratio
        if (aspectRatioDiff > 10) {
            warnings.push(`Aspect ratio mismatch (${aspectRatioDiff.toFixed(1)}% difference)`);
 ...
 */
export function detectBestFit(sourceWidth, sourceHeight, targetWidth, targetHeight) {
