...
import { createBuildArgs, createFrameArgs, createExportArgs, createInitArgs, createSpecsArgs, createValidateArgs, createCleanArgs, createLocalizeArgs, createPresetsArgs, createFontsArgs, createTemplateArgs, createQuickstartArgs, createWizardArgs } from './cli-options.js';
...
async function detectDefaultConfigVersion() {
    try {
        const { config } = await readAppshotConfig();
        return detectConfigVersion(config);
    }
    catch {
        return undefined;
    }
}
async function registerConfigTool(server, defaultConfigVersion) {
    const baseSchema = {
        configPath: z.string().optional().describe('Path to appshot config file or project directory'),
...
function registerWizardTool(server) {
    const inputSchema = z.object({
    ...
    server.registerTool('appshot_wizard', {
        ...
        const wizardArgs = createWizardArgs(args);
